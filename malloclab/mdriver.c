...
int main(int argc, char **argv) {
  ...
  /* Evaluate student's mm malloc package */
  for (i=0; i < num_tracefiles; i++) {
    trace = read_trace(tracedir, tracefiles[i]);
...
static trace_t *read_trace(char *tracedir, char *filename) {
  ...
  ret_val = fscanf(tracefile, "%d", &(trace->sugg_heapsize)); /* not used */
  if (ret_val != 1) app_error("fscanf error 1 in read_trace");
  ret_val = fscanf(tracefile, "%d", &(trace->num_ids));
  if (ret_val != 1) app_error("fscanf error 2 in read_trace");
  ret_val = fscanf(tracefile, "%d", &(trace->num_ops));
  if (ret_val != 1) app_error("fscanf error 3 in read_trace");
  ret_val = fscanf(tracefile, "%d", &(trace->weight));        /* not used */
  if (ret_val != 1) app_error("fscanf error 4 in read_trace");
    
  /* We'll store each request line in the trace in this array */
  trace->ops = (traceop_t *)malloc(trace->num_ops * sizeof(traceop_t));
  if (trace->ops == NULL)
    unix_error("malloc 2 failed in read_trace");

  /* We'll keep an array of pointers to the allocated blocks here... */
  trace->blocks = (char **)malloc(trace->num_ids * sizeof(char *));
  if (trace->blocks == NULL)
    unix_error("malloc 3 failed in read_trace");

  /* ... along with the corresponding byte sizes of each block */
  trace->block_sizes = (size_t *)malloc(trace->num_ids * sizeof(size_t));
  if (trace->block_sizes == NULL)
  ...
  while (fscanf(tracefile, "%s", type) != EOF) {
    switch(type[0]) {
      case 'a':
        ret_val = fscanf(tracefile, "%u %u", &index, &size);
        if (ret_val != 2) app_error("fscanf error 5 in read_trace");
      ...
      case 'r':
        ret_val = fscanf(tracefile, "%u %u", &index, &size);
        if (ret_val != 2) app_error("fscanf error 6 in read_trace");
        ...
        break;
      case 'f':
        ret_val = fscanf(tracefile, "%ud", &index);
        if (ret_val != 1) app_error("fscanf error 7 in read_trace");
...
/*
 * eval_mm_speed - This is the function that is used by fsecs()
...
/* 
 * eval_libc_speed - This is the function that is used by fsecs() to
...
static void printresults(int n, stats_t *stats) {
  ...
  for (i=0; i < n; i++) {
    if (stats[i].valid) {
      printf("%2d%10s%5.0f%%%8.0f%10.7f %6.0f\n", 
             i,
             "yes",
             stats[i].util*100.0,
             stats[i].ops,
  ...
  if (errors == 0) {
    printf("%12s%5.0f%%%8.0f%10.7f %6.0f\n", 
           "Total       ",
           (util/n)*100.0,
           ops, 
           secs,
